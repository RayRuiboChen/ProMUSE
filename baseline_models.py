...
class Transformer(nn.Module):
    def __init__(self, input_size=200, hidden_size=200, input_emb_dim=5, max_length=200, num_layer=6):
        ...
        self.position_emb = nn.Embedding(max_length+1, input_size)
        self.word_lienar = nn.Linear(input_emb_dim, input_size)
        ...
        self.bos_emb = nn.Parameter(torch.zeros(1, 1, input_emb_dim))
        self.init()
    ...
    def forward(self, history, early_exit=False, get_cls=False, get_final_layer=False):
        '''

        :param history: [N,20*10,5]
        :param early_exit:
        :return:
        '''
        # chlov = x[:, :60].reshape(-1, 12, 5)
        ...
        batch_size, seq_len = history.size(0), self.max_length + 1
        device = history.device

        input = torch.cat((self.bos_emb.repeat(batch_size, 1, 1), history), dim=1)
        ...
        if get_final_layer:
            return self.encoder(emb, word_mask)
        elif early_exit:
            hidden = self.encoder(emb, word_mask)
            return hidden[:, 0, :]

        elif get_cls:
            cls_emb = self.encoder(emb, word_mask, get_cls=True)
            return cls_emb
        else:
            hidden = self.encoder(emb, word_mask)

            result = self.w_out(self.dropout(hidden[:, 0, :])).view((-1))
            return result
...
class trm_Encoder(nn.Module):
    ...
    def forward(self, x, mask, get_cls=False):
        ...
        for layer in self.layers:
            ...
            if get_cls:
                cls_vecs.append(x[:, 0, :])
        if get_cls:
            return torch.stack(cls_vecs, dim=1)  # [bsz,6,200]
        else:
            return self.norm(x)
